...
class Detector:
    ...
    def postprocess(
        ...
        # Extract predictions
        # YOLOv8 Format: [x_center, y_center, width, height, class_1, class_2, ..., class_80]
        # Note: YOLOv8 does NOT have separate objectness score, only class scores!
        # Total columns = 4 (bbox) + num_classes (80 for COCO)
        
        box_coords = boxes[:, :4]  # [x_center, y_center, width, height]
        class_scores = boxes[:, 4:]  # Class probabilities [num_preds, num_classes]
        ...
        # In YOLOv8, output values are already in [0,1] range (sigmoid applied during export)
        # But let's check and apply if needed
        if np.max(class_confidences) > 1.0:
            # Raw logits - apply sigmoid
            class_scores = 1.0 / (1.0 + np.exp(-class_scores))
            class_confidences = np.max(class_scores, axis=1)
        
        # Use class confidence as the detection confidence
        ...
        keep_indices = self.nms(boxes_for_nms, confidences_filtered, self.iou_threshold)
        ...
        for idx in keep_indices:
            ...
            detection = Detection(
                ...
                confidence=float(confidences_filtered[idx]),
                class_id=class_id,
